...
//Connection Settings @0-0F4B3539
...
//Initialize Common Variables @0-55637FFF
...
$CCProjectDesign = "medibio_template";
...
$CCSDesign = "";
define("CCS_ENCRYPTION_KEY_FOR_COOKIE", 'ttjfRV28a32Nsmq2');
...
function CCSetALCookie($login, $password) {
    ...
    CCSetCookie("invt_bmLogin", $result, time() + CCS_EXPIRATION_DATE, "/", "", false);
}
//End CCSetALCookie

//CCRefreshALCookie @0-37EEFA39
function CCRefreshALCookie($expirationDate) {
    if (CCS_SLIDING_EXPIRATION) {
        if (($expirationDate - (CCS_EXPIRATION_DATE / 2)) > time()) {
            list($login, $password, $expDate) = CCParseALCookie("invt_bmLogin");
...
//CCCheckAutoLoginCookies @0-137CB3DC
function CCCheckAutoLoginCookies() {
    if (!CCGetUserLogin()) {
        $parts = CCParseALCookie("invt_bmLogin");
...
//CCLoginUser @0-795CB90F
function CCLoginUser($login, $password)
{
    CCLogoutUser();
    $db = new clsDBConnection1();
    $SQL = "SELECT id, rol, password FROM users WHERE username=" . $db->ToSQL($login, ccsText) . " AND password=" . $db->ToSQL($password, ccsText);
    $db->query($SQL);
    $Result = $db->next_record();
    if ($Result) {
        CCSetSession("UserID", $db->f("id"));
        CCSetSession("UserLogin", $login);
        CCSetSession("GroupID", $db->f("rol"));
